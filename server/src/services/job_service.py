...
class JobService:
    ...
    def process(self, spotify, job_id, user_id):
        try:
            ...
            if result := tools_process_job(spotify, job_dict):
                return jsonify({'status': 'success', 'message': 'Job processed successfully.'}), 200
    ...
    def process_scheduled_jobs(self):
        ...
        for job in jobs:
            ...
            if scheduled_time == current_hour:
                ...
                if job.last_updated and now.timestamp() - job.last_updated > 1814400:
                    print(
                ...
                try:
                    ...
                    access_token = self.spotify_service.refresh_token_if_expired(
                        token.token_info)
                    ...
                    result = self.process(spotify, job.id, user_id)

                    # Checking if result is a tuple
                    if isinstance(result, tuple) and result[1] != 200:
                        raise Exception(
                            f"Job processing failed with message: {result[0]['message']}")
